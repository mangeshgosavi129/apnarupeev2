...
function mapCompanyData(application) {
    ...
    const signatoryKyc = signatory.kyc || signatory.kycData || {};
    const signatoryAadhaar = signatoryKyc.aadhaar || {};
    const signatoryPan = signatoryKyc.pan || {};
    ...
    // Company PAN
    const companyPan = company.pan || business.pan || application.companyPan || '';
    ...
    return {
        ...
        name: signatoryName || companyName,
        ...
        applicantName: signatoryName || companyName,
        ...
        persons: directors.slice(0, 5).map((d, index) => {
            const dKyc = d.kyc || d.kycData || {};
            const dAadhaar = dKyc.aadhaar || {};
            const dPan = dKyc.pan || {};
            return {
                name: d.name || '',
                mobile: d.phone || d.mobile || '',
                email: d.email || '',
                address: formatAddress(dAadhaar.data?.address || dAadhaar.address || ''),
                pan: dPan.number || d.panNumber || '',
                aadhaar: formatAadhaar(dAadhaar.maskedNumber || d.aadhaarMasked || ''),
                din: d.din || '',
        ...
        allDirectors: directors.map((d, index) => {
            ...
            return {
                ...
                pan: dPan.number || d.panNumber || '',
                din: d.din || '',
                aadhaarLast4: (dAadhaar.maskedNumber || d.aadhaarMasked || '').slice(-4),
                designation: d.designation || 'Director',
                isSignatory: d.isSignatory || false
            };
        }),
